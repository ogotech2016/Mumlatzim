    ...
    {		
       ...
       $vPassword = $_POST['vPassword'];
        ...
        if($vUserName != '' && $vPassword != ''){
           ...
           $sql = "select * from members WHERE vUserName = '".$vUserName."' and vPassword = '".$vPassword."'";
            $result = $obj->MySQLSelect($sql); exit;
            ...
            if(count($result) > 0){            	
                ...
                header("location:".$tconfig["tsite_url"]."index.php?file=c-dashboard&var_msg_login=$var_msg");
                exit;
            }else{
                $var_msg = "Invalid Username or Password";
            	header("location:".$tconfig["tsite_url"]."index.php?file=c-login&var_err_msg_login=$var_msg");
                exit;
            }
        }else{
            $var_msg = "Invalid Username or Password";
...
//            echo $result[0]['vEmail'];
 ...
 //           exit;
            ...
            $var_msg = "Password sent successfully on your given email";
                header("location:".$tconfig["tsite_url"]."index.php?file=c-login&var_msg_pass=$var_msg");
                exit;
        }else{
            //echo '1'; exit;
            
            $var_msg = "Email not found. Please try again.";
                header("location:".$tconfig["tsite_url"]."index.php?file=c-login&var_err_msg_pass=$var_msg");
                exit;
        }
    }
?>