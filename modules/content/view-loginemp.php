...
<?php    
    if($_POST['action'] == 'chk_login')
    {
        ...
        if($vUserName != '' && $vPassword != '' && $eType != 'user'){
            ...
            if(count($result) > 0){
                ...
                {
                        setcookie ("emp_login_cookie", $vUserName, time()+2592000);
                        setcookie ("emp_password_cookie", $_POST['vPassword'], time()+2592000);
                }
                else
                {
                        setcookie ("emp_login_cookie", "", time());
                        setcookie ("emp_password_cookie", "", time());
                }
                
                $var_msg = "You login Successfully.";
                header("location:".$tconfig["tsite_url"]."index.php?file=c-empreports&var_msg=$var_msg");
                exit;
            }else{
                $var_msg = "Invalid Username or Password";
            	header("location:".$tconfig["tsite_url"]."index.php?file=c-loginemp&var_err_msg=$var_msg");
                exit;
            }
        }else{
            $var_msg = "Invalid Username or Password";
...
//            echo $result[0]['vEmail'];
 ...
 //           exit;
            ...
            $var_msg = "Password sent successfully on your given email";
                header("location:".$tconfig["tsite_url"]."index.php?file=c-loginemp&var_msg=$var_msg");
                exit;
        }else{
            //echo '1'; exit;
